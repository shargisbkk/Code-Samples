...
namespace WinFormsApp1
{
    ...
    public partial class frmCalculator : Form
    {
        ...
        public void set_Operand(object sender, EventArgs e)
        {
            ...
            {
                double result = memCalc.RepeatLast();
                rtxtDisplay.Text = result.ToString();
                isOperationPerformed = true;
            }
            else
            {
                memCalc.SetOperation(operation, double.Parse(rtxtDisplay.Text));
                isOperationPerformed = true;
            }
        ...
        public void calculate(object sender, EventArgs e)
        {
            ...
            try
            {
                if (lastPressed == "=")
                {
                    result = memCalc.RepeatLast();
                }
                else
                {
                    double secondValue = double.Parse(rtxtDisplay.Text);
                    result = memCalc.Calculate(secondValue);
                }
                rtxtDisplay.Text = result.ToString();
        ...
        {
            double.TryParse(rtxtDisplay.Text, out double value);
            value = memCalc.squareRoot(value);
            rtxtDisplay.Text = value.ToString();
        }
        ...
        {
            double.TryParse(rtxtDisplay.Text, out double value);
            value = memCalc.reciprical(value);
            rtxtDisplay.Text = value.ToString();
        }
        ...
        {
            memCalc.memoryValue = double.Parse(rtxtDisplay.Text);
            rtxtMemDisplay.Text = "M";
        }

        private void btnMemRecall_Click(object sender, EventArgs e)
        {
            rtxtDisplay.Text = memCalc.memoryValue.ToString();
        }

        private void btnMemAdd_Click(object sender, EventArgs e)
        {
            memCalc.AddToMemory(double.Parse(rtxtDisplay.Text));
            rtxtMemDisplay.Text = "M";
        }
